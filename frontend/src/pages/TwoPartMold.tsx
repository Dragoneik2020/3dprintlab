...
export default function TwoPartMold() {
  const { uploadedFile, isProcessing, error, config } = useStore();
  const { generateTwoPartMold } = useMoldGenerator();
  ...
  return (
    <div style={{ display: 'grid', gridTemplateColumns: isMobile ? '1fr' : '1fr 360px', gap: isMobile ? 12 : 24 }}>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: isMobile ? 12 : 16 }}>
        ...
        >
          {isProcessing ? '⏳ Procesando...' : '⚡ Generar Molde'}
        </button>
        {error && (
